    ...
    def draw4(self, drawer, resources, state, ampm, followxy):
        ...
        if self.getDigit():
            ...
            if self._timeType is None or self._timeType == 0:
                ...
                return self.getDigit().draw4(drawer, resources, hours, minimumDigits=2, paddingZeroLength=2)
            elif self._timeType == 1:
                return self.getDigit().draw4(drawer, resources, state.getTime().minute, minimumDigits=2, paddingZeroLength=2, followxy=followxy, padding_zero=True)
            elif self._timeType == 2:
                if not state.getScreenIdle():
                    return self.getDigit().draw4(drawer, resources, state.getTime().second, minimumDigits=2, paddingZeroLength=2, followxy=followxy, padding_zero=True)
